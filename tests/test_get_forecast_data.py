...
class Weather_get_forecast_data(unittest.TestCase):
    ...
    def test_hour_on(self, mock_update):
        ...
        self.assertEqual(self.dwd_weather.get_forecast_data(
            WeatherDataType.PRECIPITATION, test_time), 1.23, )
    ...
    def test_end_of_day(self, mock_update):
        ...
        self.assertEqual(self.dwd_weather.get_forecast_data(
            WeatherDataType.PRECIPITATION, test_time), 8.76, )

    @patch("simple_dwd_weatherforecast.dwdforecast.Weather.update", return_value=None)
    def test_midnight(self, mock_update):
        test_time = datetime(2020, 11, 7)
        self.assertEqual(self.dwd_weather.get_forecast_data(
            WeatherDataType.PRECIPITATION, test_time), 7.65, )
    ...
    def test_after_midnight_longer(self, mock_update):
        ...
        self.assertEqual(self.dwd_weather.get_forecast_data(
            WeatherDataType.PRECIPITATION, test_time), 6.54, )
